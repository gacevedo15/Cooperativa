...
public class ResumenAnual{
     ...
     * se recorren los productos de la cooperativa
     * si el TipoProducto está en el HashMap beneficioCooperativaPorProducto, se printa el tipo de producto y su beneficio,
     * en caso contrario, se printa el tipo de producto y 0.0f
     */
    ...
    /**
     * Imprime la evolución de los precios de referencia para cada producto, ordenados por fecha de forma ascendente.
     * Solo imprime los precios del año especificado en la variable anno.
     */
    public void printEvolucionPreciosReferencia() {
        System.out.println("Evolución de los precios de referencia:");

        // Crear un comparador que ordene las fechas de forma ascendente
        Comparator<LocalDate> comparadorFechas = Comparator.naturalOrder();

        for (Producto producto : productos) {
            System.out.println(producto.getTipo());
            //Si el historiaValorReferenciaPorKg no tiene entradas con el año del resumen, se printa un mensaje
            if (!producto.getHistorialValorReferenciaPorKg().keySet().stream().anyMatch(fecha -> fecha.getYear() == anno)) {
                System.out.println("  No ha sido actualizado el precio de referencia para este año.");
            }else{
                // Crear un TreeMap ordenado por fecha
                TreeMap<LocalDate, Float> historialOrdenado = new TreeMap<>(comparadorFechas);
                historialOrdenado.putAll(producto.getHistorialValorReferenciaPorKg());

                // Imprimir los precios ordenados por fecha
                for (Map.Entry<LocalDate, Float> entry : historialOrdenado.entrySet()) {
                    LocalDate fecha = entry.getKey();
                    Float precio = entry.getValue();
                    if (fecha.getYear() == anno) {
                        System.out.println("  " + fecha + ": " + TipoCooperativa.df.format(precio) + " €/Kg");
                    }
                }
